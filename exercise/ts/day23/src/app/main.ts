...
function askQuestion() {
  readline.question(
    'Enter a command (ascend (a), descend (d), park (p), or quit (q)): ',
    (command: string) => {
      switch (command) {
        case 'ascend':
        case 'a':
          try {
            ...
            if (
              e instanceof ReindeersNeedRestException ||
              e instanceof SleighNotStartedException
            ) {
